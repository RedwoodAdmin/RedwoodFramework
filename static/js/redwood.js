...
function Redwood(options) {
	...
	r.__connect__ = function() {
		...
		r.ws.onmessage = function(ws_msg) {
			...
			if (msg.Key === "__queue_start__") {
				...
				if(!r.page_refresh && r.period !== undefined){
					r.send("__page_refresh__");
				}
			}
			r.__handle_msg__(msg);
			if (msg.Key === "__queue_end__") {
				r.page_refresh = false;
			}
		}
	...
	r.recv_self = function(key, f) {
		r.recv(key, function(msg){
	...
	r.finish_load = function(f) {
		r.recv_self("__page_refresh__", f);
	};
	...
	r.page_refresh = false;
	r.recv_self("__page_refresh__", function (msg) {
	...
	r.add_points = function(points, period, user_id) {
		...
		if (r.period_points[period] !== undefined && r.period_points[period][user_id] !== undefined) {
			...
			r.period_points[period][user_id] = r.period_points[period][user_id] + parseFloat(points);
			if (user_id === r.user_id && period === r.period) {
				r.points = r.period_points[period][user_id];
			}
		}
	...
	var get_period_callbacks = [];
	r.recv('__get_period__', function(msg) {
		var callback = get_period_callbacks.shift();
		if(callback) {
			callback(msg);
		}
	});
	...
	r.get_period = function(period, callback) {
		get_period_callbacks.push(callback);
		r.send('__get_period__', { period: period });
	};
	
	var member_sync_statuses = {};
	var group_synced_callbacks = [];
	var member_synced_callbacks = [];
	r.recv('__member_synced__', function(msg){
		if(isNullOrUndefined(r.group)){
			return;
		}
		if(Object.size(member_sync_statuses) === 0){
			for(var userId in r.groups){
				var group = r.groups[userId];
				if(group === r.group){
					member_sync_statuses[userId] = {synced: false};
				}
			}
		}
		member_sync_statuses[msg.Sender].synced = true;
		member_sync_statuses[msg.Sender].data = msg.Value;
		for(var i = 0; i < member_synced_callbacks.length; i++){
			var callback = member_synced_callbacks[i];
			if(!isNullOrUndefined(callback)){
				callback(member_sync_statuses);
			}
		}
		var allSynced = true;
		for(var userId in member_sync_statuses){
			var member = member_sync_statuses[userId];
			if(!member.synced)
				allSynced = false;
		}
		if(allSynced){
			var callback = group_synced_callbacks.shift();
			member_synced_callbacks.shift();
			if(!isNullOrUndefined(callback)){
				callback(member_sync_statuses);
			}
			member_sync_statuses = {};
		}
	});
	r.on_group_synced = function(group_synced_callback, member_synced_callback){
		group_synced_callbacks.push(group_synced_callback);
		member_synced_callbacks.push(member_synced_callback);
		r.send('__member_synced__');
	};
	...
	$("[data-next-period]").each(function() {
		...
		widget.click(function() {
			widget.attr("disabled", "disabled");
			r.send("data_next_period_clicked");
			return false;
		});
		r.recv("data_next_period_clicked", function(msg){
			if(msg.Sender === r.user_id){
				widget.attr("disabled", "disabled");
				r.on_group_synced(function(){
